...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const heroCalcTeam = document.getElementById('hero-calc-team');
    const heroCalcDeal = document.getElementById('hero-calc-deal');
    const heroCalcHours = document.getElementById('hero-calc-hours');
    const heroCalcSolution = document.getElementById('hero-calc-solution');

    const heroCalcTeamValue = document.getElementById('hero-calc-team-value');
    const heroCalcHoursValue = document.getElementById('hero-calc-hours-value');

    const heroCalcSavings = document.getElementById('hero-calc-savings');
    const heroCalcTime = document.getElementById('hero-calc-time');
    const heroCalcProductivity = document.getElementById('hero-calc-productivity');
    const heroCalcVsDev = document.getElementById('hero-calc-vs-dev');
    const heroCalcRoi = document.getElementById('hero-calc-roi');

    function calculateHeroROI() {
        if (!heroCalcTeam || !heroCalcDeal || !heroCalcHours) return;

        const teamSize = parseInt(heroCalcTeam.value) || 10;
        const avgDeal = parseInt(heroCalcDeal.value) || 25000;
        const hoursWasted = parseInt(heroCalcHours.value) || 8;
        const currentSolution = heroCalcSolution ? heroCalcSolution.value : 'spreadsheets';

        // Update display values
        if (heroCalcTeamValue) {
            heroCalcTeamValue.textContent = teamSize === 1 ? '1 person' : teamSize + ' people';
        }
        if (heroCalcHoursValue) {
            heroCalcHoursValue.textContent = hoursWasted === 1 ? '1 hour' : hoursWasted + ' hours';
        }

        // Enhanced calculation logic
        const timeSavedPerWeek = hoursWasted * 0.75 * teamSize; // 75% efficiency gain
        const timeSavedAnnual = Math.round(timeSavedPerWeek * WEEKS_PER_YEAR);
        const productivityValue = timeSavedAnnual * HOURLY_RATE;
        ...
        const opportunityFactor = Math.min(teamSize * 0.05, 0.3); // 5% per team member, max 30%
        const dealSizeBoost = avgDeal * 0.15 * teamSize; // 15% deal size improvement per person
        const opportunityValue = dealSizeBoost * 4; // Assume 4 deals per year per person
        
        const devSavings = DEVELOPER_COST - GLUE_COST;
        const toolSavings = currentCost - GLUE_COST;
        const totalSavings = productivityValue + devSavings + toolSavings + opportunityValue;
        const roi = Math.round((totalSavings / GLUE_COST) * 100);

        // Update display
        if (heroCalcSavings) {
            heroCalcSavings.textContent = '$' + Math.round(totalSavings).toLocaleString();
        }
        if (heroCalcTime) {
            heroCalcTime.textContent = timeSavedAnnual.toLocaleString() + ' hours';
        }
        if (heroCalcProductivity) {
            heroCalcProductivity.textContent = '$' + Math.round(productivityValue + opportunityValue).toLocaleString();
        }
        if (heroCalcVsDev) {
            heroCalcVsDev.textContent = '$' + devSavings.toLocaleString();
        }
        if (heroCalcRoi) {
            heroCalcRoi.textContent = roi.toLocaleString() + '%';
        }

        // Add smooth animation to value changes
        const resultElements = [heroCalcSavings, heroCalcTime, heroCalcProductivity, heroCalcVsDev, heroCalcRoi];
    ...
    // Add event listeners for hero calculator
    if (heroCalcTeam) heroCalcTeam.addEventListener('input', calculateHeroROI);
    if (heroCalcDeal) heroCalcDeal.addEventListener('input', calculateHeroROI);
    if (heroCalcHours) heroCalcHours.addEventListener('input', calculateHeroROI);
    if (heroCalcSolution) heroCalcSolution.addEventListener('change', calculateHeroROI);

    // Initial calculation for hero calculator
    calculateHeroROI();
